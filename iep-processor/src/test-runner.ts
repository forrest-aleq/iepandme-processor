import { processIEPDocument, type ProcessingResult } from './index.js';
  ...
  const processPromises = testFiles.map(async (filePath, index) => {
    ...
    try {
      ...
      if (result.success && result.data) {
        ...
        if (result.usage) {
          console.log(chalk.blue(`   🔢 Total Tokens: ${result.usage.total_tokens.toLocaleString()} tokens`));
          console.log(chalk.green(`   💰 Cost: $${result.usage.cost_usd.toFixed(4)}`));
        }
...
function printEnhancedTestSummary(results: TestResult[], totalTime: number): void {
  ...
  if (results.length > 0) {
    ...
    results.forEach(result => {
      ...
      if (result.usage) {
        console.log(chalk.blue(`   Usage: ${result.usage.total_tokens.toLocaleString()} tokens | Cost: $${result.usage.cost_usd.toFixed(4)}`));
      }
